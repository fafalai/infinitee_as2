...
var replaceEscapes = function (s) {
	return s.replace(encregex, function (m) {
		return encodings[m];
	})
};
...
if (typeof xmlParser === 'undefined') xmlParser = require("xml-parser");
...
function colRefToNumber (colRef) {
  // e.g. A = 0, B = 1
  var cc;
  var idx = 0;
  var d = colRef;
...
function numberToColRef (num) {
  // e.g. (0 => A), (25 => Z), (26 => AA), (52 => BA), (702 => AAA)
  var div = parseInt(num + 1);
  var colName = "";
  var mod;

  while (div > 0) {
    mod = (div - 1) % 26;
    colName = String.fromCharCode(mod + 65) + colName;
...
/**
 * Takes a formula and breaks into components consisting of
 * cell references and everything else. The components are stored
 * in an ordered array, ordered in terms of layout (left to right).
 * This is so that building shared formulae is easier.
 *
 * @param formula
 * @param orgColNum
 * @param orgRowNum
 * @returns {Array}
 */
function parseReferences(formula, orgColNum, orgRowNum) {
  //console.log("parsing: " + formula);
  formula = replaceEscapes(formula);
  var tokenTracker = getTokens(formula);
  var specialRefArray = [];
  var currentString = "";
  while (tokenTracker.moveNext()) {
    var currentToken = tokenTracker.current();
    if (currentToken.type == TOK_TYPE_SUBEXPR) {
      //console.log("subexp: " + currentToken);
      if (currentToken.subtype === TOK_SUBTYPE_START) {
        //console.log("subexpr start, append a '('");
        currentString += "(";
      }

      else if (currentToken.subtype === TOK_SUBTYPE_STOP) {
        //logger.debug("subexpr stop, append a ')'");
        currentString += ")";
      }
    }
    else if (currentToken.type === TOK_TYPE_FUNCTION) {
      var lowercaseValue = currentToken.value.toLowerCase();
      if (currentToken.subtype === TOK_SUBTYPE_START) {
        //console.log("func start, put func name and open bracket");
        currentString += currentToken.value + "(";
      }
      else if (currentToken.subtype === TOK_SUBTYPE_STOP) {
        //console.log("func stop, close bracket");
        currentString += ")";
      }
    }
    else if (currentToken.type === TOK_TYPE_OPERAND
        && currentToken.subtype === TOK_SUBTYPE_RANGE
        && currentToken.value.match(/(\w+!)?\$?[A-Z]\$?[0-9]+/) !== null) { // If a cell reference
      specialRefArray.push(currentString); //push current string
      currentString = "";
      // insert an 2 item array. first item is col ref, second item is row ref
      // if is a string, then when thing is used, will be verbatim
      // else if a number, then this will be the change relative to the original
      var parts = currentToken.value.split(/(\$?\d+)/);
      var colNum, rowNum;
      if (parts[0][0] !== '$') {
        colNum = colRefToNumber(parts[0]) - orgColNum;
      } else {
        colNum = parts[0];
      }
      if (parts[1][0] !== '$') {
        rowNum = parseInt(parts[1] - 1) - orgRowNum;
      } else {
        rowNum = parts[1];
      }
      specialRefArray.push([colNum, rowNum]);
    } else {
      currentString += currentToken.value;
    }
  }
  // push what ever is left
  specialRefArray.push(currentString);
  return specialRefArray;
}

function buildSharedFormula (colNum, rowNum, specialRefArray) {
    var finalFormula = "";
    var rowNum = rowNum + 1;
    for (var item in specialRefArray) {
      item = specialRefArray[item];
      // if it is a relative reference (refere to parseReferences function that builds this array for more info)
      if (Object.prototype.toString.call(item) === '[object Array]') {
        var processedRef = "";
        if (typeof(item[0]) === "string") {
          processedRef += item[0];
        } else { // assume that is is col number
          var num;
          //console.log('base rel col = ' + item[0]);
          //console.log('cur col = ' + colNum);
          num = item[0] + colNum;
          processedRef += numberToColRef(num); // does this work with backwards shared formulas
        }
        if (typeof(item[1]) === "string") {
          processedRef += item[1];
        } else {
          var num;
          //console.log('base rel row = ' + item[1]);
          //console.log('cur row = ' + rowNum);
          num = item[1] + rowNum;
          processedRef += num.toString();
        }
        finalFormula += processedRef;
      } else {
        finalFormula += item;
      }
    }
...
return function parse_ws_xml_data(sdata, s, opts, guess) {
	...
	for(var marr = sdata.split(rowregex), mt = 0, marrlen = marr.length; mt != marrlen; ++mt) {
		...
		for(ri = typeof tag.r === 'undefined' ? 0 : 1; ri != cells.length; ++ri) {
			...
			//console.log("tagc = " + tagc);
			if((cref=d.match(match_v))!== null && cref[1] !== '') p.v=unescapexml(cref[1]);
			//console.log("d = " + d);
			var xmlObj = xmlParser(d);
			if(opts.cellFormula && xmlObj !== undefined && xmlObj.root !== undefined && xmlObj.root.name !== undefined && xmlObj.root.name === 'f') {
				//console.log("formula found");
				if (xmlObj.root.attributes.t !== undefined
					&& xmlObj.root.attributes.t === 'shared') {
					if (xmlObj.root.content !== undefined) {
						//console.log("undefined shared");
						//console.log("content = " + xmlObj.root.content);
						sharedFormulae[xmlObj.root.attributes.si] = parseReferences(xmlObj.root.content, tagc, tagr-1);
            //console.log("content formula: " + xmlObj.root.content);
            //console.log("parseReferences: " + parseReferences(xmlObj.root.content, tagc, tagr-1))
					}
					//console.log("getting si " + xmlObj.root.attributes.si);
          p.f = buildSharedFormula(tagc, tagr-1, sharedFormulae[xmlObj.root.attributes.si]);
          //console.log("buildSharedFormula: " + p.f);
				} else {
					p.f = replaceEscapes(xmlObj.root.content);
				}
				//console.log('p.f = ' + p.f);
			}
...
function parse_wb_xml(data, opts) {
	var wb = { AppVersion:{}, WBProps:{}, WBView:[], Sheets:[], CalcPr:{}, NamedCells:{}, xmlns: "" };
	...
	data.match(tagregex).forEach(function xml_wb(x) {
		...
		switch(strip_ns(y[0])) {
			...
			case '<definedNames>': case '<definedNames':
      pass=true;
      break;
			case '</definedNames>': pass=false; break;
			/* 18.2.5    definedName CT_DefinedName + */
      case '<definedName':
        //var xmlObj = xmlParser(x);
        //var name = xmlObj.root.attributes.name;
        //var cellRef = "";
        //var sheetName = "SS";
        ////wb.NamedCells[sheetName][cellRef] = name;
        //var co = matchtag("definedName");
        //console.log(x.match(co));
        //console.log("x: " + x);
        //console.log("y: " + y);
        //console.log("data: " + data.match(co));
        //console.log(co);
        //console.log(xmlObj);
        //console.log("found defined name: " + xmlObj.root.attributes.name);
      case '<definedName/>': case '</definedName>': break;
  ...
  console.log("looking definedNames");
  //var x = data;
  var xmlObj = xmlParser(data);
  for (var cc in xmlObj.root.children){
    var name = xmlObj.root.children[cc].name;
    if (name == "sheets") {
      var sheetsObj = xmlObj.root.children[cc].children;
      for (var jj in sheetsObj) {
        var sheet = sheetsObj[jj].attributes.name;
        console.log("sheet: " + sheet);
        wb.NamedCells[sheet] = {};
      }
    }
    if (name == "definedNames"){
      var definedNamesObj = xmlObj.root.children[cc].children;
      for (var kk in definedNamesObj){
        var fullCellRef = definedNamesObj[kk].content;
        var cellName = definedNamesObj[kk].attributes.name;
        if (cellName.indexOf("Print_Titles") > -1 || fullCellRef === '#REF!'
		|| fullCellRef.indexOf("!") === -1 || cellName.indexOf("Print_Area") > -1){
		  console.log("definedNames: ignoring name '"  + cellName + "', content '" + fullCellRef + "'");
          continue
        }
        fullCellRef = fullCellRef.split("!");
        var cellRef = fullCellRef[1].replace(/\$/g, "");
        var sheetName = fullCellRef[0];
        //console.log("sheetName: " + sheetName);
        //console.log("cellRef: " + cellRef);
        if (wb.NamedCells[sheetName] === undefined){
          sheetName = sheetName.replace(/'/g, "");
          //console.log("sheetName without aps: " + sheetName);
        } else {
	      if (wb.NamedCells[sheetName][cellRef] === undefined) {
			  wb.NamedCells[sheetName][cellRef] = cellName;
		  }	else {
			  console.log("named cell of " + cellName + " already exists!");
		  }
		}
      }
...
function parse_zip(zip, opts) {
	console.log("YAYO local version file 2");
	make_ssf(SSF);
	opts = opts || {};
	fix_read_opts(opts);
	reset_cp();
  ...
  console.log('wb.NamedCells = ' + wb.NamedCells);
