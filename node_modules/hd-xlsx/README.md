...
```
# First time
git clone git@bitbucket.org:hinaudigital/js-xlsx.git
cd js-xlsx
npm install
...
# To build and test
make clean && make dist && make test

# For development workflow
make clean && make dist && npm pack && tar xf hd-xlsx-<VERSION-NUMBER>.tgz && cp -r package/* ~/<LOCATION-TO-PENSOLVE-LOCAL-CLONE>/.meteor/local/isopacks/npm-container/npm/node_modules/hd-xlsx

# To publish publicly to NPM (needed for production release)
vi package.json # bump up patch version number
npm publish . --access public
```

## Efficient testing

Pass `{WTF: true}` as second parameter to `readFile` to throw unexpected error.
